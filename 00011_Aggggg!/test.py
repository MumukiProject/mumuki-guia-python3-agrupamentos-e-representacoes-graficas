class Test(unittest.TestCase):
  ...
  def test_proporcao_cinemas_comerciais_estatais_é_um_DataFrame(self):
    self.assertEquals(type(proporcao_cinemas_comerciais_estatais), pd.DataFrame)
    
  def test_gera_o_DataFrame_correto(self):
    indexados = proporcao_cinemas_comerciais_estatais.set_index("province")
    self.assertTrue(len(proporcao_cinemas_comerciais_estatais) == 5, "deve ser do tamanho correto")    
